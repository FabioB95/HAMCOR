"""
run_mrk335.py  —  Applica QCORONA a Mrk 335 con dati XMM reali.

Lag misurato da EPIC-pn: -91.9 +/- 43.4 s = 1.46 +/- 0.69 r_g/c
Osservazione: XMM-Newton ObsID 0306870101 (2006, alto flusso)
Bande: soft 0.3-1.0 keV vs hard 1.5-4.0 keV
Frequenze: 0.2-0.7 mHz (banda reverberation, Kara+2013)
"""
...
# ── parametri Mrk 335 da XMM ─────────────────────────────────────────────────
M_BH       = 1.3e7    # M_sun  (Chainakun+2015, MNRAS 452)
R_G_C      = 63.0     # s per r_g/c con M = 1.3e7 M_sun
LAG_OBS_S  = 91.9     # s  (valore assoluto, banda 0.2-0.7 mHz)
LAG_ERR_S  = 43.4     # s
LAG_OBS_RG = LAG_OBS_S / R_G_C    # 1.46 r_g/c
LAG_ERR_RG = LAG_ERR_S / R_G_C    # 0.69 r_g/c
R_OBS      = 0.3      # reflection fraction (Chainakun+2015)
L_X        = 1.0e43   # erg/s  (alto flusso 2006)
INCL       = 45.0     # gradi  (Chainakun+2015)
...
print("=" * 60)
print("QCORONA — Mrk 335 (XMM ObsID 0306870101, 2006)")
print("=" * 60)
...
# ── costruisci psi_obs da lag scalare ─────────────────────────────────────────
# Per dati reali non abbiamo la transfer function osservata direttamente.
# Approssimiamo con una Gaussiana centrata sul lag misurato (LAG_OBS_RG)
# e larghezza sigma = errore del lag (LAG_ERR_RG).
# Questa e' la prior piu' conservativa compatibile con la misura.
T_MIN  = max(0.0, float(LM.min()))
T_MAX  = 15.0   # r_g/c — range fisico per Mrk 335 (lag ~1.5 r_g/c)
...
print(f"\n  psi_obs: Gaussian centred at {LAG_OBS_RG:.2f} r_g/c "
...
for seed in range(N_STARTS):
    ...
    w0  = rng.exponential(size=GRID.n_cells)
    w0 /= w0.sum()
    s0  = CoronalState(
        emissivity   = w0,
...
# col 1: mappa R-z emissivita' coronale
...
if len(sig) > 1:
    ax2.set_xlim(max(0.0, t_c[sig[0]] - 1.0), t_c[sig[-1]] + 3.0)
...
fig.suptitle(
    'HAMCOR applied to Mrk 335 — XMM ObsID 0306870101 (2006)',
    fontsize=9.5, y=0.98,
)

os.makedirs('results', exist_ok=True)
fig.savefig('results/fig_mrk335.pdf', dpi=300,
            bbox_inches='tight', facecolor='white')
plt.close(fig)
print("\nSaved: results/fig_mrk335.pdf")
...
tex_lines = [
    r'% HAMCOR Mrk 335 transfer function — pgfplots source',
    r'% Packages: \usepackage{pgfplots}',
    r'%           \pgfplotsset{compat=1.18}',
    r'',
    r'\begin{tikzpicture}',
    r'\begin{axis}[',
    r'  width=6cm, height=4.5cm,',
    r'  xlabel={$\Delta t\;[r_g/c]$},',
    r'  ylabel={$\psi(t)$},',
    r'  title={Mrk~335 transfer function},',
    r'  ymin=0,',
...
with open('results/fig_mrk335.tex', 'w', encoding='utf-8') as f:
    f.write('\n'.join(tex_lines))
print("Saved: results/fig_mrk335.tex")

# ── summary finale ────────────────────────────────────────────────────────────
print()
print("=" * 60)
print("SUMMARY — HAMCOR fit: Mrk 335")
print("=" * 60)
print(f"  Lag obs:    {LAG_OBS_S:.1f} +/- {LAG_ERR_S:.1f} s  "
      f"= {LAG_OBS_RG:.2f} +/- {LAG_ERR_RG:.2f} r_g/c")
...
print(f"  N iter:     {best.n_iterations}")
